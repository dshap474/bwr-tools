...
class DeFiLlamaAPI:
    """A wrapper for the DeFi Llama API.

    This class provides comprehensive methods to fetch DeFi-related data including TVL,
    stablecoins, DEX volumes, derivatives, and more from the DeFi Llama API.

    Attributes
    ----------
        base_url (str): Base URL for the DeFi Llama API.
        coins_base_url (str): Base URL for the coins-specific endpoints.
        api_key (str): API key loaded from environment variables (DEFILLAMA_API_KEY).

    """

    def __init__(self, base_url: str = "https://api.llama.fi"):
        """Initialize the DeFi Llama API wrapper.

        Args:
        ----
            base_url (str, optional): Base URL for the API. Defaults to "https://api.llama.fi".

        """
        try:
            # Load environment variables from .env file
            # First try to load from the current directory, then from the script's directory
            env_path = os.path.join(os.path.dirname(__file__), '.env')
            if os.path.exists(env_path):
                load_dotenv(env_path)
            else:
                load_dotenv()  # Try to load from current working directory
            
            # Get the API key from environment variables
            self.api_key = os.getenv('DEFILLAMA_API_KEY')
    ...
    def print_all_functions(self) -> None:
        """Print a list of all available functions in this class."""
        try:
            functions = [func for func in dir(self) if callable(getattr(self, func)) and not func.startswith("__")]
            for _func in functions:
                pass
        except Exception:
            pass

    def get_all_protocols(self) -> pd.DataFrame | None:
        """List all protocols on DeFi Llama along with their TVL.

        Returns:
        -------
            Optional[pd.DataFrame]: DataFrame containing protocols and their TVL data,
            sorted by TVL in descending order, with columns:
                - name: Protocol name
                - slug: Protocol slug
        ...
        try:
            ...
            result_df = df[["name", "slug", "tvl", "mcap", "market_share"]]
            return result_df
    ...
    def get_protocol_tvl(self, protocol):
        ...
        -------
            dict: Dictionary containing:
                ...
                - current_tvl: Current total TVL

        Example:
        -------
            >>> client = defi_llama()
            >>> aave_tvl = client.get_protocol_tvl('aave')
            >>> # Plot historical TVL
            >>> aave_tvl['tvl'].plot(y='tvl', title='Aave TVL Over Time')

        """
        if not protocol:
            return {"error": "Must enter a protocol"}
        ...
        if response.status_code == 200:
            data = response.json()
            result = {"meta": {k: v for k, v in data.items() if not isinstance(v, list)}}
            ...
            if "tokens" in data:
                ...
                for token_data in data["tokens"]:
                    if "tokens" in token_data and isinstance(token_data["tokens"], list):
                        token_df = pd.DataFrame(token_data["tokens"])
                        if not token_df.empty and "date" in token_df.columns:
                            token_df["date"] = pd.to_datetime(token_df["date"], unit="s")
    ...
    def get_price_chart(self, coins, start=None, end=None, span=1000, period="24h", search_width=None):
                        ...
                        all_prices.append(
                            {
                                ...
                                "datetime": pd.to_datetime(price_point["timestamp"], unit="s"),
                            }
                        )
    ...
    def get_earliest_price(self, coins):
        ...
        if response.status_code == 200:
            ...
            return {"error": "Internal server error. The API might be experiencing issues."}
        else:
            return {"error": f"Failed: {response.status_code}"}
    ...
    def get_stablecoin_circulating_supply(self, include_prices=False):
        ...
        if response.status_code == 200:
            ...
            if "peggedAssets" in data:
                ...
                df["circulating_total"] = pd.to_numeric(df["circulating_total"], errors="coerce")

                # Extract chain-specific circulating supply
                for chain in df["chainCirculating"].iloc[0]:
                    df[f"circulating_{chain.lower()}"] = df["chainCirculating"].apply(
                        lambda x: (
                            x.get(chain, {}).get("current", {}).get("peggedUSD", 0) if isinstance(x, dict) else 0
                        )
                    )
                ...
                columns_to_keep = [
                    ...
                    "circulating_total",
                    "pegType",
                ] + [f"circulating_{chain.lower()}" for chain in df["chainCirculating"].iloc[0]]
                if include_prices:
    ...
    def get_stablecoin_charts_all(self, stablecoin_id=None):
        ...
        if response.status_code == 200:
            ...
            if not df.empty:
                ...
                pegged_currencies = df["totalCirculating"].apply(lambda x: x.keys()).explode().unique()

                # Create new columns for each pegged currency
                for currency in pegged_currencies:
                    df[f"circulating_{currency}"] = df["totalCirculating"].apply(lambda x: x.get(currency, 0))
    ...
    def get_stablecoin_asset(self, asset_id):
        ...
        if response.status_code == 200:
            ...
            if "chainBalances" in data:
                ...
                df = df.pivot(index="date", columns="chain", values="circulating_peggedUSD")
    ...
    def get_pool_chart(self, pool_id):
        ...
        if response.status_code == 200:
            ...
            if "data" in data and isinstance(data["data"], list):
                ...
                return pd.DataFrame()  # Return empty DataFrame if 'data' is not in the expected format
        return pd.DataFrame()  # Return empty DataFrame on request failure
    ...
    def get_dexs_overview(self, data_type="dailyVolume"):
        ...
        if response.status_code == 200:
            ...
            columns = ["name", "totalAllTime"] + [col for col in df.columns if col not in ["name", "totalAllTime"]]
            df = df[columns]
    ...
    def get_total_dex_volume(self, data_type="dailyVolume"):
        ...
        if response.status_code == 200:
            ...
            if "totalDataChart" in data:
                total_volume = pd.DataFrame(data["totalDataChart"], columns=["timestamp", "total_dex_volume"])
                total_volume["timestamp"] = pd.to_datetime(total_volume["timestamp"], unit="s")
            ...
            if "totalDataChartBreakdown" in data:
                ...
                df_temp = df.pivot(index="timestamp", columns="protocol", values="volume")
    ...
    def get_dex_summary(self, protocol, data_type="dailyVolume"):
        ...
        if response.status_code == 200:
            ...
            df = df.drop(["totalDataChart", "totalDataChartBreakdown"], axis=1, errors="ignore")
    ...
    def get_dex_volume(self, protocol, data_type="dailyVolume"):
        ...
        if response.status_code == 200:
            ...
            def extract_protocol_volumes(row):
                volumes = {}
                for _chain, protocols in row.items():
            ...
            total_volume = pd.DataFrame(data["totalDataChart"], columns=["timestamp", "total_volume"])
            total_volume["timestamp"] = pd.to_datetime(total_volume["timestamp"], unit="s")
            total_volume.set_index("timestamp", inplace=True)
        ...
        if response.status_code == 200:
            ...
            if "totalDataChartBreakdown" in data:
                ...
                if all_data:
                    ...
                    protocol_volumes = df.pivot(index="timestamp", columns="protocol", values="volume")
                    protocol_volumes.columns = [
                        f"{col.lower().replace(' ', '_')}_volume" for col in protocol_volumes.columns
                    ]
    ...
    def get_total_crypto_fees_or_revenue(self, data_type="dailyFees"):
        ...
        if response.status_code == 200:
            ...
            # Add totalDataChart as the first column after the index
            if "totalDataChart" in data:
                total_fees = pd.DataFrame(data["totalDataChart"], columns=["timestamp", "total_crypto_fees"])
                total_fees["timestamp"] = pd.to_datetime(total_fees["timestamp"], unit="s")
            ...
            if "totalDataChartBreakdown" in data:
                ...
                for entry in data["totalDataChartBreakdown"]:
                    ...
                    for protocol, fee in fees.items():
                        all_data.append({"timestamp": timestamp, "protocol": protocol, "fee": fee})
    ...
    def get_chain_fees_or_revenue(self, chain, data_type="dailyFees"):
        ...
        if response.status_code == 200:
            ...
            if "totalDataChart" in data:
                ...
                total_data["timestamp"] = pd.to_datetime(total_data["timestamp"], unit="s")
            ...
            if "totalDataChartBreakdown" in data:
                ...
                # Pivot the DataFrame to have protocols as columns
                df_temp = df.pivot(index="timestamp", columns="protocol", values="value")
    ...
    def get_protocol_fees_and_revenue(self, protocol):
        ...
        def process_data(data, column_prefix):
            df = pd.DataFrame(data["totalDataChart"], columns=["timestamp", f"{column_prefix}_daily"])
    ...
    def export_dict_to_excel(self, data_dict: dict, output_folder: str, file_name: str, start_row: int = 0) -> None:
        ...
        with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
            ...
            if isinstance(data_dict, pd.DataFrame):
                ...
                data_dict.to_excel(writer, sheet_name=sheet_name, startrow=start_row, index=True)

                # Set column widths
                worksheet = writer.sheets[sheet_name]
                for idx, col in enumerate(data_dict.columns):
                    max_len = max(data_dict[col].astype(str).map(len).max(), len(col))
                    worksheet.set_column(idx + 1, idx + 1, max_len + 2)  # +2 for some padding
    ...
    def name_to_slug(self, df_to_convert, exclude_cols=None):
        ...
        name_to_slug = dict(zip(df_mapping["name"], df_mapping["slug"], strict=False))

        # Get list of columns to convert
        cols_to_convert = df_to_convert.columns
        if exclude_cols:
            cols_to_convert = [col for col in cols_to_convert if col not in exclude_cols]
...
# df = dl.get_chain_dex_volume('solana')
